 ...
 *
 * This is a template file that can be used to facilitate the creation of a QSPI
 * flash driver for a not-yet supported flash part.
 *
...
QSPI_SECTION __attribute__((unused)) static inline uint8_t flash_XXX_read_custom_config_register(void)
{
...
QSPI_SECTION static inline void flash_XXX_write_custom_config_register(uint8_t value)
{
        __DBG_QSPI_VOLATILE__ uint8_t status;
...
QSPI_SECTION static bool flash_XXX_is_suspended(void)
{
        __DBG_QSPI_VOLATILE__ uint8_t status;
...
QSPI_SECTION static void flash_XXX_initialize(uint8_t device_type, uint8_t device_density)
{
        /* Handle 24/32 bit addressing modes, enable QUAD mode, etc */
}

QSPI_SECTION static void flash_XXX_deactivate_command_entry_mode(void)
{
        /* Implement extra steps required. e.g. Re-enable XIP mode */
}

QSPI_SECTION static void flash_XXX_sys_clock_cfg(sys_clk_t sys_clk)
{
...
QSPI_SECTION static uint8_t flash_XXX_get_dummy_bytes(void)
{
        /*
