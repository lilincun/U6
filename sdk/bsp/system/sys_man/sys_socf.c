...
{
        socf_get_soc();
        //printf("socf_timer_cb\r\n");
        if (pm_get_sleep_mode() <= pm_mode_idle) {
               //printf("socf_timer_cb if\r\n");
 ...
 *
 */
extern usb_charger_state_t usb_charger_state;//sku
extern usb_charging_state_t charging_state;//sku
static void socf_task(void *pvParameters)
{
        ...
        xSocfReport = OS_TIMER_CREATE("SOC_Report",
                                OS_MS_2_TICKS(SOCF_PT_REPORT_TIME),//1s
                                OS_TIMER_FAIL,
