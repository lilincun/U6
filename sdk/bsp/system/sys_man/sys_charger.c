...
usb_charger_state_t usb_charger_state;          //sku

//PRIVILEGED_DATA static usb_charging_state_t charging_state;
usb_charging_state_t charging_state;            //sku

void usb_attach_cb(void)
{
      printf("usb_attach_cb111111111111111111111111111111\r\n");
      //printf("usb_charger_state = %d\r\n", (int16_t)usb_charger_state);//2
      //printf("charging_state = %d\r\n",(int)charging_state);//0 off charge
      //printf("000000000000000\r\n");
}
void usb_detach_cb(void)
{
        printf("usb_detach_cb22222222222222222222222222\r\n");
        //printf("usb_charger_state = %d\r\n", (int16_t)usb_charger_state);//8
        //printf("charging_state = %d\r\n",(int)charging_state);//3
        //printf("000000000000000\r\n");
}
void usb_precharging(void)
{
        //printf("usb_precharging333333333333333333333333333\r\n");
        //printf("usb_charger_state = %d\r\n", (int16_t)usb_charger_state);
        //printf("charging_state = %d\r\n",(int)charging_state);
        //printf("000000000000000\r\n");
}
...
{
        printf("usb_charging444444444444444444444444444444444\r\n");
        //printf("usb_charger_state = %d\r\n", (int16_t)usb_charger_state);//9
        //printf("charging_state = %d\r\n",(int)charging_state);//3
        //printf("000000000000000\r\n");
}
...
static void usb_attach(void)
{
        ...
        if (hw_charger_check_vbus()) {
                //printf("usb_attach_critical 1\r\n");
                usb_attach_critical();
        } else {
               // printf("hw_charger_check_vbus no\r\n");
                timer_cnt = 0;
...
static void print_statistics(uint32_t time, int mode)
{
        ...
        hw_uart_send(HW_UART1, str_buf, strlen(str_buf)+1, NULL, NULL);//sku
}
#endif
...
static void print_ntc(int mode)
{
        ...
        //hw_uart_send(HW_UART2, str_buf, strlen(str_buf)+1, NULL, NULL);
        hw_uart_send(HW_UART1, str_buf, strlen(str_buf)+1, NULL, NULL);
...
void usb_charger_init(void)
{
        OS_BASE_TYPE status;
#if ((DEBUG_USB_CHARGER_FSM == 1) && (DEBUG_USB_CHARGER_PRINT == 1))
        const uint32_t stack_size = 2 * configMINIMAL_STACK_SIZE * OS_STACK_WORD_SIZE;//2*100*4
#else
