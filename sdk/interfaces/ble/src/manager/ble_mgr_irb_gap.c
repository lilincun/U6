...
void irb_ble_handler_gap_adv_start_cmd(OS_IRB *irb)
{
        ...
        /* Replace command IRB with response */
...
void irb_ble_handler_gapm_connect_cmp_evt(ble_gtl_msg_t *gtl)
{
        ...
        /* Setup IRB with new event and fill it */
        evt = irb_ble_init_evt(&irb, BLE_EVT_GAP_CONNECTION_COMPLETED, sizeof(*evt));
...
void irb_ble_handler_gap_connect_cmd(OS_IRB *irb)
{
        ...
        gcmd->ce_len_min = dg_configBLE_CONN_EVENT_LENGTH_MIN;
        gcmd->ce_len_max = dg_configBLE_CONN_EVENT_LENGTH_MAX;
        gcmd->nb_peers = 1;
...
void irb_ble_handler_gap_bond_req_evt(ble_gtl_msg_t *gtl)
{
        ...
        case GAPC_TK_EXCH:
        {
                if (ind->data.tk_type == GAP_TK_DISPLAY) {
                        ...
                        evt = irb_ble_init_evt(&irb, BLE_EVT_GAP_NUMERIC_REQUEST, sizeof(*evt));
                        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
...
{
        const irb_ble_gap_numeric_reply_cmd_t *cmd = (void *) irb->ptr_buf;
        ...
        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GAP_NUMERIC_REPLY_CMD, sizeof(*rsp));
        rsp->status = BLE_STATUS_OK;
...
static void gapc_encrypt_complete(ble_gtl_msg_t *gtl, void *param)
{
        struct gapc_cmp_evt *gevt = (void *) gtl->param;
        OS_IRB *irb = (OS_IRB *) param;
        ble_evt_hdr_t *irb_msg = irb->ptr_buf;

        /* Check current operation */
        if (irb_msg->evt_code == BLE_EVT_GAP_SET_SEC_LEVEL_FAILED) {
                ble_evt_gap_set_sec_level_failed_t *evt =
                        (ble_evt_gap_set_sec_level_failed_t *) irb_msg;
        ...
        } else if (irb_msg->evt_code == BLE_EVT_GAP_SECURITY_REQUEST) {
...
static bool encrypt_conn_using_ltk(uint16_t conn_idx, uint8_t auth, OS_IRB *irb)
{
        irb_ble_stack_msg_t *gmsg;
        ...
        ble_gtl_waitqueue_add(conn_idx, GAPC_CMP_EVT, GAPC_ENCRYPT, gapc_encrypt_complete, irb);
        ble_gtl_send(gmsg);
...
void irb_ble_handler_gap_security_ind_evt(ble_gtl_msg_t *gtl)
{
        ...
        if (!encrypt_conn_using_ltk(conn_idx, ind->auth, &irb)) {
                /* Send to event queue */
                ble_mgr_event_queue_send(&irb, OS_QUEUE_FOREVER);
        }
}
...
void irb_ble_handler_gap_set_sec_level_cmd(OS_IRB *irb)
{
        ...
        if (master) {
                ...
                if (!encrypt_conn_using_ltk(conn_idx, mitm ? GAP_AUTH_MITM : 0, &irb)) {
