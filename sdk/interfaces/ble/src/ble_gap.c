...
ble_error_t ble_gap_address_set(const own_address_t *address, uint16_t renew_dur)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_address_set_cmd)) {
...
ble_error_t ble_gap_device_name_set(const char *name, att_perm_t perm)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_device_name_set_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_appearance_set(gap_appearance_t appearance, att_perm_t perm)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_appearance_set_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_per_pref_conn_params_set(const gap_conn_params_t *conn_params)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_ppcp_set_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_adv_start(gap_conn_mode_t adv_type)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_adv_start_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_adv_stop(void)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_adv_stop_cmd)) {
                return BLE_ERROR_FAILED;
        }
                             ...
                             uint8_t scan_rsp_data_len, const uint8_t *scan_rsp_data)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_adv_data_set_cmd)) {
                return BLE_ERROR_FAILED;
        }
                           ...
                           uint16_t window, bool filt_wlist, bool filt_dupl)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_scan_start_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_scan_stop(void)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_scan_stop_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_connect(const bd_address_t *peer_addr, const gap_conn_params_t *conn_params)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_connect_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_connect_cancel(void)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_connect_cancel_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_disconnect(uint16_t conn_idx, ble_hci_error_t reason)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_disconnect_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_conn_rssi_get(uint16_t conn_idx, int8_t *conn_rssi)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_conn_rssi_get_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_role_set(const gap_role_t role)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_role_set_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_mtu_size_set(uint16_t mtu_size)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_mtu_size_set_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_channel_map_set(const uint64_t chnl_map)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_channel_map_set_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_conn_param_update(uint16_t conn_idx, const gap_conn_params_t *conn_params)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_conn_param_update_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_conn_param_update_reply(uint16_t conn_idx, bool accept)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_conn_param_update_reply_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_pair(uint16_t conn_idx, bool bond)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_pair_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_pair_reply(uint16_t conn_idx, bool accept, bool bond)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_pair_reply_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_passkey_reply(uint16_t conn_idx, bool accept, uint32_t passkey)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_passkey_reply_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
{
        irb_ble_gap_numeric_reply_cmd_t *cmd;
        irb_ble_gap_numeric_reply_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;

        /* setup IRB with new message and fill it */
        cmd = alloc_ble_msg(IRB_BLE_GAP_NUMERIC_REPLY_CMD, sizeof(*cmd));
        cmd->conn_idx = conn_idx;
        cmd->accept = accept;

        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_numeric_reply_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_unpair(const bd_address_t *addr)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_unpair_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_set_sec_level(uint16_t conn_idx, gap_sec_level_t level)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_set_sec_level_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_skip_latency(uint16_t conn_idx, bool enable)
{
        ...
        cmd->conn_idx = conn_idx;
        cmd->enable = enable;
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_skip_latency_cmd)) {
                return BLE_ERROR_FAILED;
        }
...
ble_error_t ble_gap_data_length_set(uint16_t conn_idx, uint16_t tx_length, uint16_t tx_time)
{
        ...
        if (!irb_execute(cmd, (void **) &rsp, irb_ble_handler_gap_data_length_set_cmd)) {
                return BLE_ERROR_FAILED;
        }
