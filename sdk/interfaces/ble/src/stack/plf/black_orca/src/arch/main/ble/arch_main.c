...
__RETAINED_CODE uint32_t lld_sleep_lpcycles_2_us_func(uint32_t slp_period)
{
        uint32_t dur_us;
...
__RETAINED_CODE uint32_t lld_sleep_us_2_lpcycles_func(uint32_t us)
{
        uint32_t lpcycles;
...
__RETAINED_CODE void lld_sleep_compensate_func(void)
{
        uint32_t dur_us;
        uint32_t slp_period;
        ...
        dur_us = lld_sleep_lpcycles_2_us_func(slp_period);
        ...
        case  LLM_P256_STATE_REL_ECC:
                ...
                //If Event not masked
                                ...
                                if(param_status == 0) {
                                        ...
                                        event->status = CO_ERROR_HARDWARE_FAILURE;
                                        event->dhkey[0] = param_status;        //put actual failure in first byte
                                }
