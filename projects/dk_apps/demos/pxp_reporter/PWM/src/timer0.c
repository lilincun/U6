...
static void timer0_blink_led(int count, bool bright)
{
         ...
         */
        hw_timer0_set_t0_reload(0x7FFF, 0x7FFF);//32767 * 2 = 65534
...
void timer0_slow_blink_func(void)
{
         ...
         */
        hw_timer0_set_t0_reload(0x04, 0x04); //32kÊ±ÖÓ   Êä³ö´ó¸Å4k
...
{
        timer0_blink_led(brightness, checked);
}

void timer0_light_led_func(uint8_t brightness)
{
        timer0_light_led(brightness);
}
