...
static void poll_timer_cb(OS_TIMER timer)
{
        ...
        OS_TASK_NOTIFY(task, LIGHT_TIMER_NOTIF, eSetBits);       //向sensor_task发出通知
}

void breath_task(void *params)
{
    ...
    */
    PRIVILEGED_DATA static OS_TIMER *light_timer;   //系统内存保护，这是FreeRTOS的V8.2系统新增加的事件组
    light_timer = OS_TIMER_CREATE("light_timer", 500 / OS_PERIOD_MS, OS_TIMER_SUCCESS,
                                            (void *) OS_GET_CURRENT_TASK(), poll_timer_cb);
    OS_ASSERT(light_timer);   //OS_ASSERT：判断hrw_timer是否已建立，若因各种可能原因致创建失败，则向stderr打印一条错误信息，然后程序终止运行


    // kx022_i2c_init();

    breath_light_init_start_stop(1);

    OS_TIMER_START(light_timer, 1000 / OS_PERIOD_MS);
